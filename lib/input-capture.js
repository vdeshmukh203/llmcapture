  ...
  function findMatchingRawInput(renderedText) {
    ...
    for (let i = _inputHistory.length - 1; i >= 0; i--) {
      ...
      if (normalizedRaw === normalizedRendered) {
        // FIX: splice the matched entry from history before returning it.
        // Previously the entry was left in place, allowing the same physical
        // keystroke event to be bound to multiple chain entries as rawInput
        // provenance — a material forensic defect (CP:4 and CP:6 in
        // sess_mo4itkoh shared identical rawInput, 2026-04-18).
        _inputHistory.splice(i, 1);
        return entry;
      }
