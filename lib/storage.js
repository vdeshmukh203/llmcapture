var SessionStorage = (function () {
  ...
  async function appendEntry(sessionId, role, renderedText, rawInput, options) {
    ...
    if (role === "user" && session.promptCount >= MAX_PROMPT_TURNS) {
    ...
    const turn =
      role === "user"
        ...
        : Math.max(session.promptCount, session.assistantCount + 1);
    ...
    if (
      role === "assistant" &&
      session.assistantCount >= MAX_PROMPT_TURNS &&
      !session.lockedAt
    ) {
      session.lockedAt = new Date().toISOString();
      ...
      session.events.push(
        makeEvent(
          "session_locked",
          "Reached the hard 50-turn session limit.",
          {
            maxPromptTurns: MAX_PROMPT_TURNS,
          },
          "warning"
