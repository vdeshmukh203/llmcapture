(function () {
  ...
  async function refreshStatus() {
    try {
      ...
      if (!tab) { setInactive('Inactive'); return null; }
      return new Promise(resolve => {
        chrome.tabs.sendMessage(tab.id, { type: 'GET_STATUS' }, res => {
          if (chrome.runtime.lastError || !res) { setInactive('Inactive'); resolve(null); return; }
          if (res.locked) {
  ...
  const EXPORT_SUBFOLDER = 'llmcapture/LLM_Forensic_Capture';

  function downloadJson(filename, data) {
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    chrome.downloads.download(
      { url, filename: `${EXPORT_SUBFOLDER}/${filename}`, saveAs: false },
  ...
  async function refreshAll() {
    ...
    await refreshLiveFeed(status?.sessionId || null);
  }
  ...
  async function refreshLiveFeed(activeSessionId) {
    const feed        = $('liveFeed');
    const feedHeader  = $('liveFeedHeader');
    if (!feed) return;

    // Fall back to most recent indexed session when not on an AI tab.
    ...
    if (!sessionId) {
      feed.innerHTML = '<div class="feed-empty">No session yet — click Initialize &amp; Verify to start recording.</div>';
      if (feedHeader) feedHeader.textContent = 'Live Feed';
      return;
    }

    const session = await SessionStorage.getSession(sessionId);
    if (!session || !session.entries.length) {
      feed.innerHTML = '<div class="feed-empty">Waiting for first message…</div>';
      if (feedHeader) feedHeader.textContent = 'Live Feed · 0 entries';
      return;
    }

    const MAX_SHOW    = 20;
    const total       = session.entries.length;
    const entries     = session.entries.slice(-MAX_SHOW);
    const isPartial   = total > MAX_SHOW;
    const isLive      = !!activeSessionId;

    // Preserve scroll position — only auto-scroll if user was already at bottom.
    const wasAtBottom = feed.scrollHeight - feed.scrollTop <= feed.clientHeight + 8;

    const moreRow = isPartial
      ? `<div class="feed-more">&#8943; ${total - MAX_SHOW} earlier entries not shown</div>`
      : '';

    const rows = entries.map(e => {
      const isUser    = e.role === 'user';
      const text      = e.renderedText || '';
      const preview   = text.slice(0, 200);
      const clipped   = text.length > 200;
      const time      = e.timestamp
        ? new Date(e.timestamp).toLocaleTimeString(undefined, { timeStyle: 'short' })
        : '';

      const badges = [
        e.rawInput                   ? '<span class="feed-badge feed-raw">raw</span>'  : '',
        e.status === 'ERROR'         ? '<span class="feed-badge feed-err">error</span>' : '',
        e.status === 'recovered'     ? '<span class="feed-badge feed-rec">rec</span>'   : '',
      ].join('');

      return `<div class="feed-entry feed-entry-${e.role}">
        <div class="feed-header">
          <span class="feed-role feed-role-${e.role}">${isUser ? 'YOU' : 'AI'}</span>
          <span class="feed-pos">#${e.chainPosition}</span>
          ${badges}
          <span class="feed-time">${time}</span>
        </div>
        <div class="feed-text">${escapeHtml(preview)}${clipped ? '…' : ''}</div>
      </div>`;
    }).join('');

    feed.innerHTML = moreRow + rows;

    if (feedHeader) {
      const label = isLive ? 'Live Feed' : 'Last Session';
      feedHeader.textContent = `${label} \u00b7 ${total} entr${total === 1 ? 'y' : 'ies'}`;
    }
