/**
 * capture-50q-gemini.js
 * Sends 50 questions to Gemini, waits for the extension to capture each one,
 * exports a forensic log to session_logs/, and prints a per-prompt result table.
 *
 * Usage: node tests/capture-50q-gemini.js
 * Prereq: node tests/setup-auth.js  (creates tests/auth/gemini-state.json)
 */
...
async function run() {
  if (!fs.existsSync(AUTH_FILE)) {
    console.error('No gemini-state.json — run: node tests/setup-auth.js');
    process.exit(1);
  }

  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-50q-gemini-'));
  ...
  const url = page.url();
  if (url.includes('accounts.google.com') || url.includes('signin') || url.includes('/auth/')) {
    console.error('⚠ Login redirect — cookies may be expired. Re-run: node tests/setup-auth.js');
  ...
  // FIX: Use stop-button presence/absence instead of cumulative model-response
  // count. Gemini virtualises the DOM and removes off-screen model-response
  // elements, so the count never exceeds ~3 — causing 90s timeouts from Q4
  // onward.  Stop-button detection is reliable regardless of virtualisation.
  const GEMINI_STOP_BTN = [
  ...
  const waitForAssistantReply = async (n) => {
    try {
      ...
      await page.waitForFunction(
        sel => !!document.querySelector(sel), GEMINI_STOP_BTN,
        { timeout: 15000 }
    ...
    } catch { console.log(`  ⚠ Timeout waiting for Gemini reply #${n}`); }
  };
  ...
  // ── Q1: send first, then wait for URL redirect + session creation ─────────────
  // getThreadKey() returns null on /app (no thread ID yet), so no stub session
  // is created on page load.  The real session appears after the URL changes to
  // /app/{threadId} following Q1 submission.
  console.log(`Q 1: ${QUESTIONS[0].substring(0, 55).padEnd(55)} `, '(sending)');
  await sendQuestion(QUESTIONS[0]);

  await page.waitForFunction(
    () => window.location.pathname.startsWith('/app/') && window.location.pathname.length > 5,
    { timeout: 20000 }
  ).catch(() => {});
...

  {
    ...
    if (!(await getSessionSnap())) {
      console.error('Content script did not create a Gemini session — giving up');
      await browserContext.close();
      fs.rmSync(userDataDir, { recursive: true, force: true });
      process.exit(1);
    }
  ...
  const snapQ1      = await waitForPromptCount(1, 45000);
  ...
  for (let i = 1; i < QUESTIONS.length; i++) {
    ...
    const snap      = await waitForPromptCount(n, 45000);
  ...
  console.log('\n══════════════════════════════════════════');
  console.log(' GEMINI 50Q TEST RESULTS');
  console.log('══════════════════════════════════════════');
