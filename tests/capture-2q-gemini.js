/**
 * capture-2q-gemini.js
 * Quick 2-prompt smoke test for Gemini — verifies auth and capture are working.
 * Usage: node tests/capture-2q-gemini.js
 */
...
async function run() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-2q-gemini-'));
  ...
  await waitStop(1);
  const s1 = await waitAssist(1, 20000);
  ...
  await waitStop(2);
  const s2 = await waitAssist(2, 20000);
