/**
 * capture-50q-claude.js
 * Sends 50 questions to Claude.ai, waits for the extension to capture each one,
 * exports a forensic log to session_logs/, and prints a per-prompt result table.
 *
 * Usage: node tests/capture-50q-claude.js
 * Prereq: node tests/setup-auth.js  (creates tests/auth/claude-state.json)
 *
 * NOTE: waitForAssistantReply uses the stop-button approach, NOT a DOM
 * element count — the specific assistant-message selectors in claude.js are
 * stale and match 0 elements, so counting them would time out every turn.
 */
...
async function run() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-50q-claude-'));
  ...
  // Claude: use stop-button presence/absence to detect streaming state.
  // DO NOT count assistant-message DOM elements — those selectors are stale
  // and match 0 elements, which would cause a 90s timeout on every turn.
  const waitForAssistantReply = async (n) => {
    try {
      // Wait for stop button to appear (generation starts) — short window.
      await page.waitForFunction(
        sel => !!document.querySelector(sel), STOP_BTN,
        { timeout: 15000 }
      ).catch(() => {}); // may already be gone for fast responses
      // Wait for stop button to disappear (generation complete).
      await page.waitForFunction(
        sel => !document.querySelector(sel), STOP_BTN,
        { timeout: 90000 }
      );
      await new Promise(r => setTimeout(r, 2000)); // settle: streaming token flush
    } catch { console.log(`  ⚠ Timeout waiting for Claude reply #${n}`); }
  };

  const waitForPromptCount = async (target, timeoutMs = 45000) => {
  ...
  const sendQuestion = async (text) => {
    ...
    const sendBtn = page.locator(
      'button[aria-label="Send message"], button[data-testid="send-button"], button[aria-label*="Send"]'
    ).first();
  ...
  // ── Q1: send first, then wait for Claude's URL redirect + session creation ────
...

  {
    ...
    if (!(await getSessionSnap())) {
      console.error('Content script did not create a Claude session — giving up');
      await browserContext.close();
      fs.rmSync(userDataDir, { recursive: true, force: true });
      process.exit(1);
    }
  ...
  console.log('\n══════════════════════════════════════════');
  console.log(' CLAUDE 50Q TEST RESULTS');
  console.log('══════════════════════════════════════════');
